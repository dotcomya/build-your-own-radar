...
const ENGINES = [
  {
    ...
    line: 'Tu factures une prestation, un produit, une mission.',
  ...
  },
  {
    ...
    line: 'Tes clients paient tous les mois tant qu’ils restent.',
...
function engineChooser(a, set, refresh) {
  ...
  return h('section', { class: 'panel' },
    h('div', { class: 'panel-head' },
      ...
      h('p', { class: 'panel-sub' }, "Ce choix décide de la forme de ta courbe. Tu peux en changer à tout moment : rien n'est perdu."),
    ),
    h('div', { class: 'engines' },
...
function priceCard(a, s, voc, set, refresh) {
  ...
  fields.push(textField({
    label: 'Ce que tu vends', value: a.name,
    placeholder: `Ex. ${voc.one}`,
    hint: "Le nom que tu emploierais devant un client.",
  ...
  if (engine.key !== 'abonnement') {
    ...
    fields.push(numberField({
      label: 'Ce que ça te coûte', value: a.unitCost, field: 'unitCost', suffix: '€',
      hint: 'Tout ce qui augmente quand tu vends une unité de plus : matières, sous-traitance, commission, livraison.',
  ...
  if (engine.key !== 'unitaire') {
    ...
    fields.push(numberField({
      ...
      hint: 'Hébergement, support, licences : ce que te coûte un abonné chaque mois.',
    ...
    fields.push(numberField({
      ...
      hint: 'Douze mois est la norme. Sans engagement, mets 1 et compte sur l’attrition.',
    ...
    fields.push(numberField({
      ...
      hint: 'La part de tes abonnés qui partent chaque mois. 2 à 3 % est courant ; 5 % est alarmant.',
      onInput: (v) => set({ churnMonthly: v }, 'Attrition'),
    }))
  }

  return h('section', { class: 'panel' },
    h('div', { class: 'panel-head' }, h('h2', {}, 'Tes quatre chiffres')),
...
function verdictCard(a, s, r, voc, navigate) {
  ...
  return h('section', { class: `panel verdict-panel ${bad ? 'is-bad' : ''}` },
    h('div', { class: 'panel-head' },
      h('h2', {}, bad ? 'Tu vends à perte' : 'Ce que rapporte un client'),
      h('p', { class: 'panel-sub' }, bad
        ? "Ton coût dépasse ton prix : aucun volume ne rattrapera ça."
    ...
    h('div', { class: 'panel-body' },
      ...
      clientsNeeded !== null
        ? h('div', { class: 'model-answer' },
            h('div', { class: 'model-answer-label' }, 'Pour couvrir tes charges, il te faut'),
        ...
        : h('div', { class: 'model-answer model-answer-todo' },
            ...
            h('div', { class: 'model-answer-note' }, "Saisis tes charges et ta rémunération, et Fizzy te donne le nombre exact."),
...
function unitEconomics(a, s, r, voc) {
  ...
  return h('section', { class: 'panel mt' },
    h('div', { class: 'panel-head' },
      ...
      h('p', { class: 'panel-sub' }, `Sur ${euro(price)} encaissés auprès d'un ${voc.client}, voilà ce qu'il te reste pour payer tout le reste.`),
    ),
    h('div', { class: 'panel-body' },
      ...
      h('p', { class: 'tiny muted', style: { margin: '14px 0 0' } },
        "Cette marge doit couvrir tes salaires, ton loyer, ton comptable, tes impôts — et te payer. C'est pour ça qu'une marge de 30 % n'est pas 30 % de bénéfice."),
    ),
  )
}
...
function sectorNote(sector, voc) {
  ...
  return h('div', { class: 'note plain mt' },
    h('div', { class: 'note-title' }, `Dans ton métier : ${pct(range[0], 0)} à ${pct(range[1], 0)} de marge brute`),
