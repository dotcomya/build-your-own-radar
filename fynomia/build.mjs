/**
 * Construit une version en fichier unique de Fynomia.
 *
 ...
 *
 *   node build.mjs            → dist/fynomia.html (document complet)
 *   node build.mjs --body     → dist/fynomia-body.html (sans <html>/<head>,
...
const body = `<title>Fynomia</title>
...
if (process.argv.includes('--body')) {
  writeFileSync(join(dist, 'fynomia-body.html'), body)
  console.log(`dist/fynomia-body.html — ${Math.round(Buffer.byteLength(body) / 1024)} Ko`)
} else {
...
`
  writeFileSync(join(dist, 'fynomia.html'), doc)
  console.log(`dist/fynomia.html — ${Math.round(Buffer.byteLength(doc) / 1024)} Ko`)
}
