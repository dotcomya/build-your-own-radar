...
for (const tpl of ['logiciel','conseil','ecommerce','restaurant','coiffeur']) {
  ...
  let idOk = true
  for (let y=0;y<5;y++){
    const ebitda = p.revenue[y] - p.variableCost[y] - p.external[y] - p.duties[y] - p.payroll[y] + p.grants[y]
    if (!near(ebitda, p.ebitda[y], 2)) { idOk=false; console.log(`   y${y+1} EBITDA calc=${ebitda.toFixed(0)} vs ${p.ebitda[y].toFixed(0)}`) }
    const ebit = p.ebitda[y] - p.amortisation[y]
