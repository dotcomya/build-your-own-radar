/**
 * Trois lectures du même projet : mêmes faits, écriture différente, aucun
 * personnage.
 *
 * Les lecteurs imaginés — une équipe, une salle pressée, un fonds ou une
 * banque — guident la rédaction ; ils ne sont jamais
 * nommés à l'écran, et personne n'y parle. On vérifie :
 *
 *   1. qu'aucune des trois formes ne nomme un lecteur, ne cite un
...
export default async function (t) {
  ...
  const formes = {}
  for (const mise of ['Récit', 'Diapos', 'En détail']) formes[mise] = await lire(mise)
