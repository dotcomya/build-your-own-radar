...
export default async function (t) {
  ...
  for (let k = 0; k < 16; k++) {
    ...
    else if (/Tu vends quoi/.test(q)) await p.locator('.setup-input').fill('Pizza napolitaine')
    else if (/rentre l/.test(q)) { await p.locator('.setup-choice').first().click(); await p.waitForTimeout(200); await p.locator('.setup-input').fill('14') }
    ...
    else if (/clients le premier mois/.test(q)) await p.locator('.setup-input').fill('900')
    else if (/elle te coûte quoi/.test(q)) await p.locator('.setup-cost-toggle').first().click()
