...
export default async function (t) {
  // 1. La synthèse d'origine suit les chiffres.
  {
    ...
    await t.exemple(p)
    const lire = async () => { await t.aller(p, 'tableau-de-bord', 1000); return p.evaluate(() => (document.querySelector('.plain') || {}).innerText || '') }
    const a = await lire()
    ...
    await f.fill('35000'); await f.blur(); await p.waitForTimeout(700)
    const b = await lire()
    ...
    await s.fill('150000'); await s.blur(); await p.waitForTimeout(700)
    const c = await lire()
  ...
  for (const format of ['bureau', 'telephone']) {
    ...
    await t.exemple(p)
    await t.aller(p, 'tableau-de-bord', 1000)
