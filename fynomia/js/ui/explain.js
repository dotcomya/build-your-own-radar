...
export function revenueSentence(r) {
  const p = r.pnl
  const y = 0
  const marge = p.revenue[y] > 0 ? p.ebe[y] / p.revenue[y] : 0
  const seuil = r.kpis.breakEven[y]
  const dessous = seuil && p.revenue[y] < seuil
  return `En année 1, ${euro(p.revenue[y], { compact: true })} de chiffre d'affaires laissent ${euro(p.ebe[y], { compact: true })} d'EBE, soit ${pct(marge, 0)}. `
    + (seuil
